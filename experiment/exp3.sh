...
log_dir="logs/exp3"
mkdir -p "$log_dir"
rm -f "$log_dir"/exp3.log "$log_dir"/client-*.log

echo "========================================"
echo " Experiment 3 - With Synchronization"
echo "========================================"
echo "Workers: 3"
echo "Synchronization: ON (Mutex)"
echo "Random Delay: ON (50-500 ms)"
echo "Target Resource: 10"
echo "========================================"

stdbuf -oL ../server 3 1 > >(tee "$log_dir/exp3.log") 2>&1 &
server_pid=$!
sleep 1
